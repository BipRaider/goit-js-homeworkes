...
console.log(
	'task-3',
	users.filter((user) => user.gender === 'male').reduce((accum, elem) => [ ...accum, elem.name ], [])
);
...
console.log(
	'task-4',
	users.filter((user) => user.isActive !== true).reduce((accum, elem) => [ ...accum, elem.name, elem ], [])
...
const calculateTotalBalance = (users) => users.reduce((accum, valueBalance) => accum + valueBalance.balance, 0);
...
const getUsersWithFriend = (users, friendName) =>
	users.filter((user) => user.friends.find((friend) => friend === friendName)).map((names) => names.name);
...
const getNamesSortedByFriendsCount = (users) =>
	users.sort((a, b) => a.friends.length - b.friends.length).map((names) => names.name);
